...
**현재 버전: v0.4.4 · 제작자: PSP**

![Lua 화면을 PC에서 재현한 미리보기](미리보기.png)

## 다운로드 / 설치

[설치 ZIP 다운로드](HELI3D-v0.4.4-CRASH-MODE.zip)
...
- 견적 모드(CRASH RESET): OFF는 지면 접촉 후 계속 비행, ON은 이륙 후 지면 접촉 시 자동 초기화. 기본 OFF.
...
| SOUND | ON / OFF |
| CRASH RESET | 견적 모드 ON / OFF (기본 OFF). ON이면 이륙 후 지면 접촉 시 시작 화면으로 리셋 | 
...
배포 Lua 원본을 Fengari Lua 5.3과 모의 EdgeTX API로 실행한 **104개 검사**가 통과했습니다. [검증 결과](검증결과.txt)를 제공합니다.
